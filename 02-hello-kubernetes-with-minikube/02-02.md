...
A _ConfigMap_ allows configuration files to be stored independently of a container image.  The files stored within the _ConfigMap_ can then be mounted into a container within a _Pod_ at runtime.
...
A _Secret_ allows secret files and values to be stored independent of a container image.  Much like a _ConfigMap_ but also with the ability to store the value of a key in an environment variable.
